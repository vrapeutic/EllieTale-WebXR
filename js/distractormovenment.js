AFRAME.registerComponent("moverandomly", {
  init: function () {
    ...
    const startDsMovement = function randommovenmet() {
      setTimeout(function () {
        randomIndexTarget = Math.floor(
          Math.random() * Math.floor(target.length)
        );
        ...
        if (randomIndexTarget >= target.length - 1) {
          ...
          ds.setAttribute(
            "animation",
            "property:position; to:" +
              (target[randomIndexTarget].getAttribute("position").x - 1) +
              " 0 " +
              target[randomIndexTarget].getAttribute("position").z +
              " dur:4000"
          );
