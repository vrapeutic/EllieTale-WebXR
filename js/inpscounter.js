...
AFRAME.registerComponent("inpsscore", {
  tick: function () {
    ...
    let inpusCount = document
      .querySelector("#inpsCounter")
      .getAttribute("value");
    let index = document.querySelector("#inps").getAttribute("value");
    var taskcounter = document
      .getElementById("taskcounter")
      .getAttribute("value");
    if (
      taskcounter > 1 &&
      taskcounter < document.querySelectorAll(".Box").length
    ) {
      if (this.el.sceneEl.camera) {
        ...
        var fairyPosition = new THREE.Vector3(
          fairy.getAttribute("position").x,
          fairy.getAttribute("position").y,
          fairy.getAttribute("position").z
        );
